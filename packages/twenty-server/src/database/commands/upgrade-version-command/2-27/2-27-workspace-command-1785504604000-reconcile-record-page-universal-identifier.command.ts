...
import { invalidateRecordPageReconcileCache } from 'src/database/commands/upgrade-version-command/2-27/utils/invalidate-record-page-reconcile-cache.util';
...
@RegisteredWorkspaceCommand('2.27.0', 1785504604000)
@Command({
  name: 'upgrade:2-27:reconcile-record-page-universal-identifier',
  description:
    'Re-own the default record-page stack of every object whose RECORD_PAGE layout belongs to the twenty-standard or workspace-custom application onto the engine convention: the layout gets the name-free deterministic universal identifier (getRecordPageLayoutUniversalIdentifier, object identifier + RECORD_PAGE discriminator), its standard/workspace-custom tabs and widgets get title-keyed derived identifiers, the FIELDS_WIDGET record-page view referenced by the layout FIELDS widget gets the name-free derived identifier (object identifier + FIELDS_WIDGET key) plus the reserved key backfill, its view fields get getSystemViewFieldUniversalIdentifier keyed on the application of the field they display (note the incremental-path rows hold underived v4 identifiers, not parent-derived values), and everything is flagged isSystemSideEffect. View field groups are flagged and re-owned only when authored by twenty-standard: groups are also created by the user-facing CRUD API, and user-created groups must keep isSystemSideEffect false or they lose their only non-cascade cleanup. App-authored tabs attached to the standard layout (e.g. fireflies, call-recorder) are left untouched. Children reference their parents by primary key and widget universal configurations resolve view primary keys at flat-maps build time, so the re-own is a lossless update followed by a full flat-maps closure invalidation. Record-page stacks of other applications are handled by the backfill-application-record-page command.',
})
export class ReconcileRecordPageUniversalIdentifierCommand extends ProvisionedWorkspaceCommandRunner {
  constructor(
