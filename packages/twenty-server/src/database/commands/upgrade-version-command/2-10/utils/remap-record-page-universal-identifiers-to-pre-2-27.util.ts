...
// The 2-27 record-page reconcile re-owns every standard record-page view,
...
// scheme. Upgrade commands registered BEFORE 2-27 run on workspaces whose rows
...
// pre-2.27 literals before being compared with or committed to the workspace.
...
export const PRE_2_27_RECORD_PAGE_UNIVERSAL_IDENTIFIER_BY_DERIVED: Record<
  string,
  string
> = {
...
export const toPre227RecordPageUniversalIdentifier = (
  universalIdentifier: string,
): string =>
  PRE_2_27_RECORD_PAGE_UNIVERSAL_IDENTIFIER_BY_DERIVED[universalIdentifier] ??
  universalIdentifier;
...
// its pre-2.27 literal. Serialization-based on purpose: universal identifiers
// are unique tokens, so a textual replacement is exact and covers nested jsonb.
export const remapRecordPageUniversalIdentifiersToPre227 = <T>(
  flatEntity: T,
): T => {
  let serialized = JSON.stringify(flatEntity);

  for (const [derivedUniversalIdentifier, pre227UniversalIdentifier] of Object.entries(
    PRE_2_27_RECORD_PAGE_UNIVERSAL_IDENTIFIER_BY_DERIVED,
  )) {
    serialized = serialized
      .split(derivedUniversalIdentifier)
      .join(pre227UniversalIdentifier);
  }
